...
目前正式流程：選片、停格、框選主角、完整 ViT 追蹤，再輸出保留原始背景的影片。ViT 不可跳過。
...
- EdgeTAM WebGPU 去背候選在真實 iPhone 的 3 秒測試會讓 Safari 重新載入頁面，已從公開版撤下。
- 不得再把同一候選交給使用者重測；必須先降低模型初始化的記憶體峰值，並在隔離候選中驗證。
- 未經使用者同意不得再次部署去背功能。
- 目前公開版已恢復為使用者指定的 V19（commit `81519e6` 的內容與 V19 `6419eb4` 相同）。V20 Pose Landmarker 讓手腳更差且旁人漏入沒有改善，已撤回，不得再沿該方向微調。
- 新的 V21 候選改用 MediaPipe MagicTouch 點提示物件分割：ViT 框照舊，首幀提示點放在框內軀幹，後續以已接受遮罩的中心延續同一物件；V19 Selfie Segmenter 僅作失敗回退。候選尚未發布，必須先通過建置與 iPhone 實測。
...
## 15. 2026-08-31 重要還原點與第 13 項候選

- 使用者已在真實 iPhone 驗收 V21 MagicTouch 結果為「雖不完美但可接受」，並指定為重要還原點。
- 重要 commit：`f9d2718b8fe87c82cb5c4543558fd96bc65fd2f5`。
- 重要 tag：`nivi-v21-magic-touch-important-2026-08-31`；另有 `nivi-v21-magic-touch-acceptable-2026-08-31`。兩個 tag 不得移動、覆寫或刪除。
- 第 13 項是獨立的「旁人人臉遮罩」：使用者先以原有 ViT 流程指定主角；輸出保留原片構圖與主角臉，只遮住其他人臉。
- 可選效果：柔和模糊、強力模糊、馬賽克、黑色橢圓、Emoji、自選貼紙；另有遮罩大小、強度與隱私優先設定。
- 第 13 項使用本機 MediaPipe BlazeFace Short Range；不做人臉身分辨識，不上傳影片或人臉資料，不需要帳號、後端或付費 API。
- 第 13 項的偵測器、狀態、預覽與匯出分支必須維持獨立；只有使用者選第 13 項才載入。前 12 項演算法、預設值與流程不得因它改變。
- 目前第 13 項仍是本機候選，尚未發布。未收到使用者明確說「發布」前不得推送 GitHub Pages。

### 第 13 項 V23 人頭追蹤候選

- V22（commit `7412d5c`）已發布，但真實 iPhone 實測「幾乎沒效果」；根因是只靠 BlazeFace，側臉、背面與小臉會沒有偵測框。
- V23 本機候選改為混合流程：現有 SSDLite 每 0.2 秒找出所有 `person`，依 ViT 主角框排除主角，再從其餘人物框上方推定人頭；BlazeFace 補強可見臉部；遮罩軌跡跨幀保留以降低漏遮與閃爍。
- 預設效果改為「強力模糊」。完整追蹤完成後會釋放較大的 SSDLite／TensorFlow.js 模型，再進入小型遮罩預覽或輸出，避免 iPhone 同時保留兩套模型。
- V23 尚未發布；必須先完成全部測試並取得使用者明確「發布」。
